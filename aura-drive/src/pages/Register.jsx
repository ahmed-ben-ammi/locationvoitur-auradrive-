...
import { UserPlus, Lock, Phone, ArrowRight } from 'lucide-react';
...
export default function Register() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await register({ name, CNE, password, phone });
      navigate('/login');
    } catch (err) {
  ...
  const isFormValid = name && CNE && phone && isValidPhone(phone) && password && confirmPassword && (password === confirmPassword);

  return (
    <>
      <Helmet>
        <title>Drive Car Go - {t('nav.register')}</title>
      </Helmet>
